...
@test "an absent ~/.claude/skills does not abort the seed" {
  # Regression: the skills-mirror block counts dangling links with
  #   n="$(as_user find "$CLAUDE_HOME/skills" ... 2>/dev/null | wc -l)"
  # find exits 1 when the directory does not exist. 2>/dev/null hides the
  # message but not the status, `pipefail` promotes it to the pipeline's, and
  # the command substitution makes it the assignment's -- so `set -e` killed the
  # script silently, part-way through, on every container that had no
  # ~/.claude/skills. Everything after this point (the plugin registry home-path
  # repair, the sentinel, the installers) never ran.
  #
  # A developer machine and any warm container both have the directory, which is
  ...
  [ "$status" -eq 0 ]
  # Reaching the sentinel proves execution got past the skills block rather than
  # merely surviving it.
  [ -f "$HOME/.claude/.seeded" ]
}

@test "an absent ~/.agents/skills mount with dangling links still completes" {
  # The other side of the same branch: with links present but no mount, the
  # warning path runs the same find.
  #
  # The link has to be planted in the HOST SEED, not directly in $HOME: the
  # authored-subset refresh above prunes and re-copies ~/.claude/skills from the
  # seed on every start, so anything written straight into $HOME is erased
  # before this block is reached. Seeding it is also how the real case arises --
  # `cp -a` reproduces the host's relative symlink verbatim, pointing at an
  # ~/.agents path that was never mounted.
  mkdir -p "$TEST_ROOT/host-seed/.claude/skills"
  ln -s ../../.agents/skills/tdd "$TEST_ROOT/host-seed/.claude/skills/tdd"
