...
@test "printed environment does not pin a Claude model" {
  run env VEKIL_PROXY_SOURCE_ONLY=1 VEKIL_STATE_DIR="$STATE_DIR" bash -c '
    source "$1/bin/vekil-proxy"
    print_env
  ' _ "$REPO_ROOT"
  ...
  [ "$status" -eq 0 ]
  # Model choice belongs to Claude Code (settings.json / the /model picker);
  # printing an export here would silently override it.
  [[ "$output" != *"ANTHROPIC_MODEL"* ]]
  # Both the host and devcontainer blocks are still emitted.
  [ "$(printf '%s\n' "$output" | grep -c 'ANTHROPIC_BASE_URL=')" -eq 2 ]
}
