...
@test "sd_parse_doc reads all thirteen blocks from the real catch-up doc" {
  ...
  # The three rows added after the original ten. Each anchors a block a real
  # seed was found missing entirely while the detector scored it clean, so a
  # row silently dropped here would restore that blind spot.
  [[ "$output" == *"projects/.git"* ]]
  [[ "$output" == *"GIT_BUILD_VERSION"* ]]
...
@test "sd_scan puts the real template core.excludesFile block in one paragraph" {
  ...
  [ "$status" -eq 0 ]
  # Absolute line numbers, so ANY template edit above this block shifts them --
  # the pin last moved by twelve when the count_entries helper landed upstream
  # of it. What the test actually guards is that the range stays a SINGLE
  # paragraph of the same span (260-217 == 43); if only the offset moved, re-pin
  # it, and only investigate sd_scan when the span itself changes.
  [ "$output" = "217 260" ]
}
...
t7_copy_tool() {
  ...
  # The engine lives in bin/lib/seed-drift/ and the CLI resolves it relative
  # to its own location, so the copy needs the lib tree beside it. Sabotage
  # edits land on T7_LIB files (or T7_TOOL for CLI-level sabotage), never on
  # the repository's own copies.
  T7_LIB="$BATS_TEST_TMPDIR/lib/seed-drift"
  T7_LIB_PRISTINE="$BATS_TEST_TMPDIR/lib-pristine"
  cp "$REPO_ROOT/bin/seed-drift" "$T7_TOOL"
  cp "$REPO_ROOT/bin/seed-drift" "$T7_TOOL_PRISTINE"
  mkdir -p "$BATS_TEST_TMPDIR/lib"
  cp -R "$REPO_ROOT/bin/lib/seed-drift" "$T7_LIB"
  cp -R "$REPO_ROOT/bin/lib/seed-drift" "$T7_LIB_PRISTINE"
  # Verify rather than trust: see the `cp` note above. The copies land on
  ...
  local part
  for part in extract normalize verdict report; do
    cmp -s "$REPO_ROOT/bin/lib/seed-drift/$part.sh" "$T7_LIB/$part.sh"
    cmp -s "$REPO_ROOT/bin/lib/seed-drift/$part.sh" "$T7_LIB_PRISTINE/$part.sh"
  done
  chmod +x "$T7_TOOL"
}
...
t7_assert_repo_tool_untouched() {
  ...
  local part
  for part in extract normalize verdict report; do
    cmp -s "$T7_LIB_PRISTINE/$part.sh" "$REPO_ROOT/bin/lib/seed-drift/$part.sh"
  done
}
...
@test "sabotage: dropping templated assignments without the definedness check hides an undefined WORKSPACE" {
  ...
  sed_inplace '/^  problems=\$(sd_templated_var_problems/,/^  fi$/d' "$T7_LIB/report.sh"
  # The excision must actually have happened, or this test passes vacuously.
  ! grep -q 'sd_templated_var_problems "\$sscan"' "$T7_LIB/report.sh"
  cat >"$TPL" <<'TPLEOF'
#!/usr/bin/env bash
...
@test "sabotage: reverting to a per-call (non-union) window size wrongly flags the multi-paragraph seed as thin" {
  ...
  python3 - "$T7_LIB/extract.sh" <<'PYEOF'
import sys
path = sys.argv[1]
...
@test "sabotage: raising SD_THIN_WINDOW past the seed's window size hides the note" {
  ...
  sed_inplace 's/^SD_THIN_WINDOW=5$/SD_THIN_WINDOW=1/' "$T7_LIB/report.sh"
  t7_run_tool "$T7_TOOL"
  [ "$status" -eq 0 ]
  [[ "$output" != *"window is only"* ]]
