...
@test "claude check mode reports every destination the real run links" {
  # Dry-run/apply drift is invisible to the immutability test: a link missing
  # from the check branch is a link ai-check can never report on. Assert the
  # check output names each managed destination.
  run env HOME="$HOME" PATH="/usr/bin:/bin" bash "$REPO_ROOT/ai/claude/install.sh" --check
  [ "$status" -eq 0 ]
...
SCRIPT
  ...
  run /usr/bin/env PATH="$PATH" bash -c \
    ...
    "$REPO_ROOT/ai/vekil/token-lib.sh" "$token_file"
