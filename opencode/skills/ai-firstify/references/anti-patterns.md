...
**Description:** Deploying an LLM-powered agent inside a web application (chatbot, conversational UI, or API-driven agent). Result: "Nobody used it because it was too complex and had too little context." Switched to agent skills and it worked "10 times better."
...
2. Convert each capability to an agent skill
...
**Description:** Creating custom tool-calling, prompt chaining, or agent orchestration systems instead of using the AI agent's built-in capabilities.
...
**Fix procedure:**
1. Map custom agent capabilities to built-in agent features
2. Replace custom tool-calling with agent tools (scripts/)
...
**Description:** Everything in AGENTS.md instead of using skills and references. All context loaded at once instead of progressively.
...
**How to detect:**
- Features mentioned in code but not in AGENTS.md or any plan
...
**Fix procedure:**
1. Start a new session when switching tasks
2. Document key learnings in AGENTS.md before starting fresh
3. Use compaction when context is large but still relevant
