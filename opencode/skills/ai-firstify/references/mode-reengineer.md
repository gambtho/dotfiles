...
## Phase 1: Foundation
- Create or rewrite AGENTS.md (read references/project-structure.md for template)
...
- For each found: propose replacement with agent skills + sub-agents
- After user approval, remove agent infrastructure code
- Remember: "deployed an agent in a web app that nobody used because it was too complex and had too little context -- switched to agent skills and it worked 10 times better"

## Phase 3: Skill Extraction
- Identify repeated workflows (scripts run multiple times, similar prompt patterns, recurring data transformations)
- Extract each into a proper skill in `.opencode/skills/`
...
- Check if pre-built or community skills could replace custom code
...
- Audit AGENTS.md: is it focused? Under 500 lines? Well-structured?
- Move large reference documents from AGENTS.md to separate files
