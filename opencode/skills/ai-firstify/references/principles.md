...
- Keep AGENTS.md under 200 lines (ideally under 100)
...
**Summary:** Use Plan mode (Tab key) before building. Planning is cheaper than rebuilding.

**Description:** Before building anything substantial, enter plan mode. The agent will research, ask clarifying questions, propose a file structure and approach, and wait for your approval before writing code. You can iterate on the plan before a single line of code is written. In OpenCode, switch to Plan mode using the Tab key.
...
**How to detect in code:**
- No AGENTS.md (suggests no planning phase)
...
**How to fix:**
- Always use Plan mode (Tab) before building
- Write a brief plan in AGENTS.md before starting
...
**Summary:** Use a maintained AI coding agent. It improves automatically. Your custom agent won't.

**Description:** You already have an agent with tools, research, and coding abilities. Building agent infrastructure in your applications is almost never the right choice. Your custom agent will never be as smart, bulletproof, or flexible as maintained agents like OpenCode. As models improve, the agent improves automatically -- your custom agent stays stuck in time.

> **Important:** Sub-agents WITHIN the AI agent (e.g., using the Task tool or sub-agent patterns in skills) are fine and encouraged. "Don't build your own agent" means don't build a web app or deployed service with an embedded LLM agent. Using the agent's built-in multi-agent capabilities is the recommended approach.
...
**How to fix:**
- Replace embedded agents with skills
- Convert agent workflows to skill + sub-agent patterns
- Remove LLM API dependencies
- Use the agent's built-in sub-agent capabilities instead

---

## Principle 6: Stick to the Point

**Summary:** Keep context relevant. Use skills and tools for on-demand context. Keep AGENTS.md clean.

**Description:** Agents work much better when their context is limited to what is relevant in the moment. Use separate files, tools, skills, and sub-agents to provide context when relevant. Keep AGENTS.md clean and focused.

**When it applies:** Structuring AGENTS.md. Designing skills. Organizing project files.

**Common violations:**
- AGENTS.md that's an encyclopedia (500+ lines)
...
- Keep AGENTS.md focused on project overview and conventions
...
- AGENTS.md that's too sparse (under 10 lines)
...
**Summary:** Start fresh sessions for new tasks. Fresh folders. Document learnings first.

**Description:** Start with a fresh folder as context and bring in exactly what you need. During big sessions, clearing context dramatically improves performance. Before starting a new session: document useful insights in AGENTS.md so they persist.

**When it applies:** Starting a new task. Switching between different work. After long sessions.

**Common violations:**
- Never clearing context or starting fresh sessions (stale context accumulates)
- Not documenting learnings before starting a new session
- Mixing unrelated tasks in one session
- Keeping old project files when starting fresh

**How to detect in code:**
- No AGENTS.md (suggests no documentation of learnings)
...
- Document key learnings in AGENTS.md before starting fresh
