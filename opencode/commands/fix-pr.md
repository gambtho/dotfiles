---
description: Analyze a PR's open comments and failing CI checks, then produce a detailed implementation plan
agent: build
---
...
```
mkdir -p ~/.config/opencode/pr-fix-plans/{OWNER}/{REPO}
```
...
**Launch these agents in parallel on the PR's changed files using the Task tool:**
...
Create the file at: `~/.config/opencode/pr-fix-plans/{OWNER}/{REPO}/pr-{PR_NUMBER}-plan.md`
...
```
Plan generated: ~/.config/opencode/pr-fix-plans/{OWNER}/{REPO}/pr-{PR_NUMBER}-plan.md
...
```
Next steps:
  - Review the plan: Read ~/.config/opencode/pr-fix-plans/{OWNER}/{REPO}/pr-{PR_NUMBER}-plan.md
  - To start implementing, you can feed this plan back to OpenCode
```
