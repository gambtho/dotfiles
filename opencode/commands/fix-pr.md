...
```
mkdir -p ~/.opencode/pr-fix-plans/{OWNER}/{REPO}
```
...
| Code explorer | `code-explorer` | Traces execution paths and maps dependencies to understand impact of changes |
...
Create the file at: `~/.opencode/pr-fix-plans/{OWNER}/{REPO}/pr-{PR_NUMBER}-plan.md`
...
```
Plan generated: ~/.opencode/pr-fix-plans/{OWNER}/{REPO}/pr-{PR_NUMBER}-plan.md
...
```
Next steps:
  - Review the plan: Read ~/.opencode/pr-fix-plans/{OWNER}/{REPO}/pr-{PR_NUMBER}-plan.md
