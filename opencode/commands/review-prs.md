...
5. Set the output directory: `~/.opencode/pr-reviews/{OWNER}/{REPO}/`
...
1. Read `~/.opencode/pr-reviews/{OWNER}/{REPO}/learnings.md` (if it exists)
...
Read `~/.opencode/pr-reviews/{OWNER}/{REPO}/config.md` if it exists. This file contains user-defined review conventions that supplement the auto-detected checklist:
...
| Sub-agent | Task type | When to use |
|-----------|-----------|-------------|
| Silent failure hunter | `silent-failure-hunter` | PR touches error handling, catch blocks, or fallback logic |
| Comment analyzer | `comment-analyzer` | PR adds or modifies comments/docstrings |
| Type design analyzer | `type-design-analyzer` | PR introduces new types, interfaces, or structs |
| Code reviewer | `code-reviewer` | Always — catches bugs and security issues with confidence filtering |
...
Create `~/.opencode/pr-reviews/{OWNER}/{REPO}/review-{YYYY-MM-DD}.md` (if a file with today's date exists, append a counter: `review-{date}-2.md`).
...
`~/.opencode/pr-reviews/{OWNER}/{REPO}/config.md`
...
Read the existing `~/.opencode/pr-reviews/{OWNER}/{REPO}/learnings.md` (if any), then update it. Do NOT just append — consolidate and deduplicate. The file structure should be:
...
- If CONTRIBUTING.md or CLAUDE.md or AGENTS.md exist in the repo, treat their guidelines as authoritative for checklist items they cover.
