...
5. Set the output directory: `~/.config/opencode/pr-reviews/{OWNER}/{REPO}/`
   - Create it with `mkdir -p` if it does not exist
6. Print: **"Reviewing PRs for: {OWNER}/{REPO}"**

### 0c: One-Time Migration

If `~/.claude/pr-reviews/{OWNER}/{REPO}/learnings.md` exists (old location), copy it to `~/.config/opencode/pr-reviews/{OWNER}/{REPO}/learnings.md` if no learnings file already exists at the new location. Print a note that migration was performed. Also check for review files at the old location and migrate them.
...
1. Read `~/.config/opencode/pr-reviews/{OWNER}/{REPO}/learnings.md` (if it exists)
...
Read `~/.config/opencode/pr-reviews/{OWNER}/{REPO}/config.md` if it exists. This file contains user-defined review conventions that supplement the auto-detected checklist:
...
| `AGENTS.md` | Read for project-specific coding guidelines |
...
2. For the top 15 with comments (or top 5 if learnings already exist), use parallel agents (3-5 at a time, or 2 if RATE_LIMITED) via the Task tool with the @explore agent to fetch and analyze review comments:
   ```
...
**Fallback** if GraphQL fails: use the REST approach with parallel checks:
```
...
3. Categorize each PR by size and type:
   - **Lockfile-only**: All changed files match `*lock*`, `*.lock`, `go.sum`, `*.sum` — will review from diff only, no worktree
   - **Small** (< 100 lines changed): Review from diff only, no worktree
   - **Medium** (100-500 lines changed): Standard review with worktree
   - **Large** (500-1500 lines changed): Extended review with worktree, agent focuses on highest-impact files
...
5. Print a summary table of the PRs that will be reviewed (including size category) and proceed.
...
Launch review agents in batches of up to 5 at a time (or 2 if RATE_LIMITED) using the Task tool with the @general agent.
...
Create `~/.config/opencode/pr-reviews/{OWNER}/{REPO}/review-{YYYY-MM-DD}.md` (if a file with today's date exists, append a counter: `review-{date}-2.md`).
...
`~/.config/opencode/pr-reviews/{OWNER}/{REPO}/config.md`
...
Read the existing `~/.config/opencode/pr-reviews/{OWNER}/{REPO}/learnings.md` (if any), then update it. Do NOT just append — consolidate and deduplicate. The file structure should be:
...
- If CONTRIBUTING.md, AGENTS.md, or CLAUDE.md exist in the repo, treat their guidelines as authoritative for checklist items they cover.
