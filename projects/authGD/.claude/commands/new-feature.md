---
description: Start a new authGD feature using the standing team-agent flow — brainstorm, plan, spawn team with role-typed members, seed TaskList.
argument-hint: "<feature-slug> [short description]"
---

# /new-feature $ARGUMENTS

You are starting a new authGD feature. Follow this flow **strictly and in
...
2. After brainstorm completion, invoke `superpowers:writing-plans` to produce
   a written plan file the team can read. Plans live in
   `docs/superpowers/plans/` alongside the existing numbered plans.
...
- The acceptance criteria (`npm test` passes, `npm run typecheck` clean,
  `npm run test:e2e` green, UI matches screenshot)
- Which layer it belongs to, so the right teammate claims it

**authGD's dependency shape is usually the same**, so encode it rather than
rediscovering it each time:

1. Pure logic in `src/core/` + its unit test — blocks everything else
2. Schema change in `src/db/` + `npm run db:generate` — blocks services
3. Service layer in `src/services/` — blocks both the job and the page
4. Job in `src/jobs/` + worker wiring — parallel with the page
5. Page/action in `src/app/` — parallel with the job
6. E2E spec in `e2e/` — blocked on the page
...
| Need | Spawn |
|---|---|
| Worker, jobs, core logic, services, schema | `Agent({ team_name: "<slug>", name: "backend", subagent_type: "sync-engine-dev", prompt: "Claim and execute backend tasks from the team TaskList. Read team config at ~/.claude/teams/<slug>/config.json to discover peers." })` |
| Pages, server actions, OAuth routes, e2e specs | `Agent({ team_name: "<slug>", name: "frontend", subagent_type: "frontend-dev", prompt: "Claim and execute web-tier tasks from the team TaskList." })` |
...
Most authGD features touch both tiers — a new sync surface needs a job *and* a
page — so spawn `backend` and `frontend` together. Reviewer/polisher spawn later
(or via `addBlockedBy` task gating).
...
- Don't spawn more members than the work calls for. A change to one pure
  function in `src/core/` doesn't need a frontend agent.
- Don't let the web teammate call ESI/Discord/Wanderer directly. That boundary
  is the architecture; enqueue a job instead.
- Don't claim done without verification (`npm test`, `npm run typecheck`,
  `npm run test:e2e`, screenshots for UI).
