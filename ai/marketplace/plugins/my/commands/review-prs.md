...
If `~/.claude/pr-reviews/learnings.md` exists at the old flat path AND `~/.claude/pr-reviews/{OWNER}/{REPO}/learnings.md` does NOT exist, move all files from the old path into `~/.claude/pr-reviews/{OWNER}/{REPO}/`. Print a note that migration was performed.
...
2. For the top 15 with comments (or top 5 if learnings already exist), use parallel agents with `model: "haiku"` (2-5 at a time, or 2 if RATE_LIMITED) to fetch and analyze review comments. Use the `Agent` tool in parallel calls. Each agent should fetch and analyze 3-5 PRs via:
   ```
...
Launch review agents in batches of up to 5 at a time (or 2 if RATE_LIMITED) using the `Agent` tool. Make all calls in a single message per batch. Select the **model per PR based on size category**:
