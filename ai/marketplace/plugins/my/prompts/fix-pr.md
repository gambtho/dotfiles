...
```
mkdir -p ~/.pi/pr-fix-plans/{OWNER}/{REPO}
```
...
When `DEEP_MODE` is set, dispatch specialized reviewers in parallel with the `subagent` tool to surface issues that reviewers and CI may have missed. Issue one sibling call per relevant role with a focused read-only `prompt`, a 3–5 word `description`, `subagent_type: deep`, and `run_in_background: true`:
...
Read `~/.dotfiles/ai/marketplace/plugins/my/references/fix-pr-plan-template.md` — it has the exact markdown structure to write, plus per-section rules (verbatim quoting of comments, blockquotes for context, omit Conflicting Feedback if no conflicts, etc.).
...
```
Plan generated: ~/.pi/pr-fix-plans/{OWNER}/{REPO}/pr-{PR_NUMBER}-plan.md
...
```
Next steps:
  - Review the plan: Read ~/.pi/pr-fix-plans/{OWNER}/{REPO}/pr-{PR_NUMBER}-plan.md
  - To start implementing, feed this plan back to Pi
```
...
- Convention detection: check for both `CLAUDE.md` and `AGENTS.md` at the project root.
