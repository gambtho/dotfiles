...
# Count entries under a path that may not exist.
#
# find(1) exits 1 for a missing path. Under `set -euo pipefail` that status
# survives `2>/dev/null` (which hides the message, not the exit code),
# `pipefail` promotes it to the pipeline's status, and a command substitution
# makes it the assignment's -- so `n="$(find ... | wc -l)"` terminates the whole
# script, silently and part-way through. Every count in this file is over an
# OPTIONAL directory, where absent means zero rather than fatal.
count_entries() {
  { as_user find "$@" 2>/dev/null || true; } | wc -l
}
...
# ---------------------------------------------------------------------------
# git >= 2.48 — ALWAYS-RUN, and built ONLY when this workspace actually needs it.
#
# A host that sets worktree.useRelativePaths (git >= 2.48) stamps
#     [core]       repositoryformatversion = 1
#     [extensions] relativeworktrees = true
# into .git/config the first time `git worktree add` runs there. That config is
# SHARED with the container over the workspace bind, and an unknown extension at
# format version 1 is a deliberate hard refusal — so an older container git
# rejects EVERY command in the workspace, not just worktree ones:
#     fatal: unknown repository extension found: relativeworktrees
#
# A newer base image is usually NOT the fix. Debian bookworm ships git 2.39,
# bookworm-backports has nothing newer, and even trixie is only 2.47. Alpine
# 3.22+ clears the bar, but moving a project to musl can break far more than it
# fixes (BEAM/native artifacts shared with a glibc host, every apt-based line in
# the Dockerfile). The image is also usually tracked and shared with upstream,
# so changing it is permanent drift for a problem created by ONE developer's
# host gitconfig. That makes this seed the right layer.
#
# Always-run because ~/.local is the ephemeral writable layer: a version-gated
# step would be skipped as "already seeded" long after a rebuild deleted it.
#
# The build is guarded by the ACTUAL failure, not by a version comparison alone.
# Most Debian-based images have a git older than 2.48, but only a workspace whose
# .git/config carries an extension this git cannot parse is broken by it —
# everyone else would be paying a compile for nothing.
#
# Non-fatal throughout: an old git still runs the project, and a container that
# refuses to start is worse than one where `git log` does.
GIT_MIN_VERSION=2.48
GIT_BUILD_VERSION=2.54.0
GIT_BUILD_SHA256=f689162364c10de79ef89aa8dbf48731eb057e34edbbd20aca510ce0154681a3
GIT_PREFIX="$SEED_HOME/.local"

# sort -V rather than field parsing: git's version strings are not uniformly
# three-component (release candidates carry an -rcN suffix), and a hand-rolled
# numeric compare orders those wrongly.
seed_git_new_enough() {
  local git_bin="$1" git_ver
  [ -x "$git_bin" ] || return 1
  git_ver="$(as_user "$git_bin" --version 2>/dev/null | awk 'NR==1 {print $3}')" || return 1
  [ -n "$git_ver" ] || return 1
  [ "$(printf '%s\n%s\n' "$GIT_MIN_VERSION" "$git_ver" | sort -V | head -n1)" = "$GIT_MIN_VERSION" ]
}

if seed_git_new_enough "$GIT_PREFIX/bin/git"; then
  echo "🌱 seed: git $(as_user "$GIT_PREFIX/bin/git" --version | awk '{print $3}') already present"
elif seed_git_new_enough "$(command -v git 2>/dev/null || echo /nonexistent)"; then
  echo "🌱 seed: git $(git --version | awk '{print $3}') already new enough (image-provided)"
else
  # Probe as the seeded user, not as root: ownership and safe.directory are
  # evaluated per-user, so a root probe can succeed where the real terminal
  # fails. Capture stderr only — a bare failure is ambiguous (an absent repo
  # fails too), and the extension message is the one symptom worth compiling for.
  GIT_EXT_ERR="$(as_user git -C "$WORKSPACE" rev-parse --git-dir 2>&1 >/dev/null || true)"
  case "$GIT_EXT_ERR" in
    *"unknown repository extension"*)
      # Collapse to one line: git reports the extension name on a second,
      # tab-indented line, and a raw embedded newline would split this into two
      # log entries where only the first carries the 🌱 prefix.
      GIT_EXT_SUMMARY="$(printf '%s' "$GIT_EXT_ERR" | tr '\n\t' '  ' | tr -s ' ')"
      echo "🌱 seed: building git $GIT_BUILD_VERSION (image git $(git --version 2>/dev/null | awk 'NR==1 {print $3}') cannot read $WORKSPACE — $GIT_EXT_SUMMARY)"
      GIT_TMP="$(as_user mktemp -d)"
      GIT_SRC="$GIT_TMP/git-$GIT_BUILD_VERSION"
      # sysconfdir=/etc, NOT the prefix default of $prefix/etc. A prefix build
      # otherwise reads system config from ~/.local/etc/gitconfig and silently
      # stops honouring /etc/gitconfig — where devcontainer images conventionally
      # put `safe.directory = <workspace>`. Losing that trades this failure for
      # "detected dubious ownership", which reads as an unrelated regression
      # rather than a consequence of the fix.
      #
      # NO_GETTEXT drops the only build dependency Debian images typically lack
      # (messages stay untranslated); NO_TCLTK skips gitk/git-gui, which a
      # headless container cannot use. Everything else git needs — a compiler,
      # make, perl, and the curl/expat/zlib/ssl headers — is already present on
      # any image carrying build-essential, so this installs no packages and
      # needs no sudo.
      #
      # Build output goes to a log: a parallel compile would bury every other
      # seed line, and the tail only matters when it fails.
      if as_user curl -fsSL --connect-timeout 10 --max-time 600 \
        -o "$GIT_TMP/git.tar.xz" \
        "https://mirrors.edge.kernel.org/pub/software/scm/git/git-${GIT_BUILD_VERSION}.tar.xz" &&
        echo "$GIT_BUILD_SHA256  $GIT_TMP/git.tar.xz" | sha256sum -c - >/dev/null 2>&1 &&
        as_user tar -xf "$GIT_TMP/git.tar.xz" -C "$GIT_TMP" &&
        as_user make -C "$GIT_SRC" -j"$(nproc 2>/dev/null || echo 2)" \
          prefix="$GIT_PREFIX" sysconfdir=/etc NO_TCLTK=1 NO_GETTEXT=1 \
          all >"$GIT_TMP/build.log" 2>&1 &&
        as_user make -C "$GIT_SRC" \
          prefix="$GIT_PREFIX" sysconfdir=/etc NO_TCLTK=1 NO_GETTEXT=1 \
          install >>"$GIT_TMP/build.log" 2>&1 &&
        seed_git_new_enough "$GIT_PREFIX/bin/git"; then
        echo "🌱 seed: git $(as_user "$GIT_PREFIX/bin/git" --version | awk '{print $3}') ready"
      else
        echo "⚠️  seed: git build failed (non-fatal; git stays at $(git --version 2>/dev/null | awk 'NR==1 {print $3}') and will refuse commands in $WORKSPACE)"
        if [ -f "$GIT_TMP/build.log" ]; then
          echo "⚠️  seed: last lines of the build log:"
          tail -n 5 "$GIT_TMP/build.log" | sed 's/^/         /'
        fi
      fi
      rm -rf "$GIT_TMP"
      ;;
    *)
      # Older than 2.48, but nothing in this workspace needs the newer format.
      # Silent on purpose: this is the common case for most projects, and a line
      # every launch about a non-problem trains the reader to skim the log.
      :
      ;;
  esac
fi
...
# ---------------------------------------------------------------------------
# SKILL LINK TARGETS (~/.agents/skills) — always-run, deliberately ABOVE the
# gate. The companion to the copy above, and the reason it is not already
# sufficient.
#
# ~/.claude/skills is frequently a MIXED tree: real directories alongside
# relative symlinks of the form `tdd -> ../../.agents/skills/tdd`, written that
# way by some skill installers. `cp -a` reproduces a symlink as a symlink and
# never follows it, so the links arrive intact and resolve to a path that was
# never mounted. The failure is silent in both directions -- `ls ~/.claude/
# skills` lists every entry, nothing errors, and the skills are simply absent
# from Claude.
#
# Copying the targets fixes all of them at once and keeps working as skills are
# added or removed host-side, which per-skill repair does not. It also has to
# happen HERE rather than by hand in a running container: the block above prunes
# ~/.claude/skills unconditionally on every start, so a skill installed over a
# dead link is erased on the next start and the dead link returns. Installers
# that test only for the directory entry then see it and skip -- which makes the
# breakage look self-healing while it is in fact stable.
#
# Same gating logic as the ~/.claude copy: the source is the host, so
# SEED_VERSION cannot observe host-side changes and must not latch this. No
# named volume mounts ~/.agents either, so the destination is ephemeral -- a
# second, independent reason not to gate it.
as_user rm -rf "$SEED_HOME/.agents/skills"
if [ -d "$SEED_AGENTS/skills" ]; then
  as_user mkdir -p "$SEED_HOME/.agents"
  as_user cp -a "$SEED_AGENTS/skills" "$SEED_HOME/.agents/skills"
  # Count what actually RESOLVES rather than what was copied: a link whose
  # target the host itself had already lost copies without complaint, and the
  # whole point of this block is that a present-but-dangling entry reads as
  # success everywhere else.
  seed_skill_dangling="$(count_entries "$CLAUDE_HOME/skills" -maxdepth 1 -xtype l)"
  if [ "$seed_skill_dangling" -eq 0 ]; then
    seed_skill_count="$(count_entries "$SEED_HOME/.agents/skills" -maxdepth 1 -mindepth 1)"
    echo "🌱 seed: mirrored ~/.agents/skills ($seed_skill_count skills; all ~/.claude/skills links resolve)"
  else
    echo "⚠️  seed: mirrored ~/.agents/skills, but $seed_skill_dangling ~/.claude/skills link(s) still dangle:"
    as_user find "$CLAUDE_HOME/skills" -maxdepth 1 -xtype l -printf '         %f -> %l\n' 2>/dev/null
  fi
else
  # Not every project mounts this, and plenty of skill trees contain no links at
  # all. Warn only when there is a real symptom to point at -- an unconditional
  # notice about an absent optional mount is noise on most projects.
  seed_skill_dangling="$(count_entries "$CLAUDE_HOME/skills" -maxdepth 1 -xtype l)"
  if [ "$seed_skill_dangling" -gt 0 ]; then
    echo "⚠️  seed: no $SEED_AGENTS/skills mount and $seed_skill_dangling ~/.claude/skills entr(ies) dangle —"
    echo "         add '~/.agents/skills:$SEED_AGENTS/skills:ro,cached' to docker-compose.override.yml"
  fi
fi
...
self_check() {
  ...
  # h2) Skill links resolving. Distinct from (h) above, which only scans
  # $WORKSPACE for links into dotfiles/projects — these live in ~/.claude/skills
  # and point at ~/.agents/skills, so that scan never sees them. A FAIL rather
  # than a WARN: a dangling entry here is invisible everywhere else. The
  # directory still lists it, nothing errors, and the skill is just quietly
  # absent from Claude.
  if [ -d "$CLAUDE_HOME/skills" ]; then
    local skill_broken
    skill_broken="$(as_user find "$CLAUDE_HOME/skills" -maxdepth 1 -xtype l -printf '%f -> %l\n' 2>/dev/null | sort)"
    if [ -z "$skill_broken" ]; then
      echo "   PASS  all ~/.claude/skills links resolve"
    else
      echo "   FAIL  dangling links in ~/.claude/skills:"
      printf '%s\n' "$skill_broken" | sed 's/^/         /'
      fails=$((fails + 1))
    fi
  fi
