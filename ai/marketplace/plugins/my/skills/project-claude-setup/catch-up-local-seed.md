...
|---|---|---|
| `~/.dotfiles` mirror | `projects/.git` | `projects/` is a nested **private** repo. Its overlay files are payload and must mirror; its `.git` is history plus a remote URL naming the repo, landing in a named volume that outlives the container and ships with any image built from it. Needs **both** halves: the anchored `--exclude='/projects/.git'` on the rsync, and the unconditional `rm -rf` after it — `cp -a` has no exclusion mechanism, and rsync *protects* an excluded path on the receiver from `--delete`, so a volume seeded before the exclusion keeps its copy forever on the exclude alone. |
...
| git build | `GIT_BUILD_VERSION` | Debian-based images ship a git older than 2.48, which cannot parse a `.git/config` carrying a newer repository extension — `fatal: unknown repository extension found: relativeworktrees`, from a host-side `git worktree` on a bind-mounted checkout. Builds a pinned, sha256-verified git into `~/.local`. Must stay **always-run**: `~/.local` is the ephemeral writable layer, so a gated step is skipped as "already seeded" long after a rebuild deleted the binary. Guarded on the actual parse failure, not the version alone; non-fatal throughout. |
...
| skill link targets | `.agents/skills` | Most of `~/.claude/skills` is **relative** symlinks of the form `tdd -> ../../.agents/skills/tdd`. `cp -a` copies the link verbatim without ever touching what it points at, so without this mirror the majority of skills dangle in the container while looking perfectly healthy on the host — and an install guard that tests entry *existence* rather than resolution reports them present. Needs the matching `~/.agents/skills:/host-seed/.agents/skills:ro,cached` mount in the override; a new Compose volume requires a **rebuild**, not a restart. |
