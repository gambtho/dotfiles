...
project's own AGENTS.md / CLAUDE.md, README, or maintainer guidance) always take
...
**unresolved decisions**. When the work warrants it, do this as an explicit pass
before implementing: state current understanding, confirmed constraints, likely
blind spots, the decisions that could materially change the implementation, and
...
Before substantial implementation, produce a concise, evidence-based plan. Lead
with the decisions most likely to need review, and keep routine mechanical edits
last:
...
follow-up, and where reviewers should focus.
