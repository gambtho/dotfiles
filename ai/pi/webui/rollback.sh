...
require_no_managed_plan_files() {
  [[ "$MANAGED_PLAN_FILE_COUNT" == 0 ]] || {
    fail 'managed Pi plan files must be removed or archived first'
    return 1
  }
}
...
validate_worktree() {
  ...
  local target_git_raw target_git status ignored_status line mode
