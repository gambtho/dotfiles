...
# shellcheck source=ai/vekil/token-lib.sh
source "$(dirname "${BASH_SOURCE[0]}")/token-lib.sh"
...
persist_restart_required() {
  ...
  validate_regular_target "$RESTART_REQUIRED_FILE"
  publish_staged_file "$STAGED_RESTART_REQUIRED" "$RESTART_REQUIRED_FILE"
  STAGED_RESTART_REQUIRED=""
}
...
install_vekil() {
  ...
  validate_regular_target "$VEKIL_BIN"
  validate_regular_target "$VERSION_FILE"
  publish_staged_file "$STAGED_BIN" "$VEKIL_BIN"
  STAGED_BIN=""
  publish_staged_file "$STAGED_VERSION" "$VERSION_FILE"
  STAGED_VERSION=""
  VEKIL_CHANGED=1
  rm -rf "$DOWNLOAD_DIR"
  DOWNLOAD_DIR=""
