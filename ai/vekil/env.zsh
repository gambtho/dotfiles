...
() {
  ...
  if (( _vekil_env_anthropic_managed )); then
    ...
    # Leave model choice to Claude Code (settings.json / the /model picker).
    # Still clear a value this function pinned previously, so shells that
    # inherited the old default stop carrying it.
    if [[ -n ${VEKIL_MANAGED_ANTHROPIC_MODEL:-} ]]; then
      unset ANTHROPIC_MODEL VEKIL_MANAGED_ANTHROPIC_MODEL
    fi
